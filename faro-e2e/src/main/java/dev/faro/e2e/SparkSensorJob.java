...
import dev.faro.spark.DeltaCaptureEventSink;
import dev.faro.spark.SparkFaro;
 ...
 * {@code NON_PERSONAL} features — to a Delta table at {@code /tmp/faro-spark-capture}.
 */
    ...
    public static void run(SparkSession spark, String tablePath) {
        FaroConfig<Reading> config = FaroConfig.<Reading>builder()
                .feature("temperature", FaroFeatureConfig.<Reading>builder()
                        .entityKey(Reading::getDeviceId)
                        .featureValue(Reading::getTemperature)
